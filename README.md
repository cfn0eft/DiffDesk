...
| 投入後サポート | **投入検証**(件数照合と✔/✖の合否判定、検証レポート)、**Data Loaderエラーファイル分析**(error.csvの失敗理由を日本語で集計+対処ヒント+失敗行だけの再投入用CSV生成)、**投入ロールバックキット**(投入前のSF値による復元用CSV+success.csvからの取り消し用delete CSV) |
| 予防・監視 | **ファイル健康診断**(列プロファイル+基準保存で「行数急変・空欄率急増・未知の値・列の増減」を自動警告)、**フォルダ監視**(`diffdesk watch` — ファイルが置かれたら自動突合検証しレポート出力) |
| 編集 | グリッド編集(セル・行・列の追加削除、ソート、表示フィルタ、Enterで下セル移動、元に戻す)、検索・置換(正規表現対応)、一括クレンジング(空白除去・全半角変換・日付統一・数字のみ抽出・**空欄を上の値で埋める**) |
| データ品質 | **表記ゆれ検出→一括統一**(「株式会社テスト/テスト(株)」等を自動グルーピング)、検証(キー重複・必須列・形式・**許可値=ピックリスト**・**数値の基準範囲**チェック。許可値は別ファイルの実値から自動生成可)、**キーなし あいまい突合**(共通キーのないファイル同士を類似度で紐づけ)、**クロス集計**(群×性別の件数確認等) |
...
| 出力 | CSV(UTF-8 / BOM付き / CP932、CRLF)、Excel(.xlsx)、色付きExcel差分レポート、差分レポートCSV、**共有用HTMLレポート**(フィルタ・検索内蔵の単一ファイル。メール添付でそのまま開ける) |
